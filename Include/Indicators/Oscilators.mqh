...
public:
   ...
   int               Deivation(void)     const { return(m_deviation); }
...
//| Constructor                                                      |
//+------------------------------------------------------------------+
CiZigZag::CiZigZag(void) : m_depth(12),
                           m_deviation(5),
                           m_backstep(3)
...
//--- check result
   if(m_handle==INVALID_HANDLE)
      return(false);
//--- indicator successfully created
   if(!Initialize(symbol,period,depth,deviation,backstep))
     {
      //--- initialization failed
      IndicatorRelease(m_handle);
      m_handle=INVALID_HANDLE;
   ...
   if(CreateBuffers(symbol,period,1))
     {
      ...
      m_name  ="ZigZag";
      m_status="("+symbol+","+PeriodDescription()+","+
               IntegerToString(depth)+","+IntegerToString(deviation)+","+
...
//+------------------------------------------------------------------+
//| Access to Main buffer of "ZigZag"                |
//+------------------------------------------------------------------+
...
//|                                                                  |
//+------------------------------------------------------------------+
class CiBIAS : public CIndicator
  {
protected:
   int               m_ma_period;
   int               m_applied;
...
//| Constructor                                                      |
//+------------------------------------------------------------------+
CiBIAS::CiBIAS(void) : m_ma_period(-1),
                       m_applied(-1)
  {
  }
//+------------------------------------------------------------------+
...
//| Create indicator "Relative Strength Index"                       |
//+------------------------------------------------------------------+
bool CiBIAS::Create(const string symbol,const ENUM_TIMEFRAMES period,const int ma_period,const int applied)
  {
//--- check history
   if(!SetSymbolPeriod(symbol,period))
      return(false);
//--- create
   m_handle=iCustom(symbol,period,"MyIndicator\\BIAS",ma_period,applied);
//--- check result
   if(m_handle==INVALID_HANDLE)
      return(false);
//--- indicator successfully created
...
//| Initialize the indicator with universal parameters               |
//+------------------------------------------------------------------+
bool CiBIAS::Initialize(const string symbol,const ENUM_TIMEFRAMES period,const int num_params,const MqlParam &params[])
  {
   return(Initialize(symbol,period,(int)params[0].integer_value,(int)params[1].integer_value));
  }
//+------------------------------------------------------------------+
//| Initialize the indicator with special parameters                 |
//+------------------------------------------------------------------+
bool CiBIAS::Initialize(const string symbol,const ENUM_TIMEFRAMES period,const int ma_period,const int applied)
  {
   if(CreateBuffers(symbol,period,1))
     {
      //--- string of status of drawing
...
//+------------------------------------------------------------------+
//| Access to buffer of "BIAS"                    |
//+------------------------------------------------------------------+
double CiBIAS::Main(const int index) const
  {
   CIndicatorBuffer *buffer=At(0);
//--- check
   if(buffer==NULL)
